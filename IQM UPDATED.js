function OnLoad(context) {
    ...
    setPEOnCreate(context);
    onloadLeave(context);
    GetLeaveBalance(context); // Changed to pass formContext instead of executionContext
    leave(context);//fprmtype not added
    BirthDayStrtEndDiff(context);
    birthday(context);
    GoodFriday(context);
}
...
/*
function autoSaveIfDirty(context) {
    var formContext = context.getFormContext();
        ...
        setTimeout(function () {
            ...
            if (formContext.data.entity.getIsDirty()) {
                formContext.data.save().then(
                    function () {
                        console.log("Record saved successfully after 2 seconds.");
                    },
                    function (error) {
                        console.error("Failed to save the record after 2 seconds: " + error.message);
                    }
                );
            } else {
                console.log("No unsaved changes, auto-save not triggered.");
            }
        }, 2000);
    } else {
        console.log("This is a new record. Auto-save is not triggered.");
    }
}*/
...
function ResubmitBtn(primaryControl) {
    ...
    var resolvecheck = formContext.getAttribute("pg_resolvecheck").getValue();

    if (stateCode === 0 && leavestatus === 140310000 && LeaveType === 7 && resolvecheck != true) {
        return true;
    }
    else {
        return false;
...
function isRecordInactive(primaryControl) {
    var formContext = primaryControl;
    var stateCode = formContext.getAttribute("statecode").getValue();
    var LeaveType = formContext.getAttribute("pg_leavetype").getValue();
    if (stateCode === 1 && LeaveType === 7) {
        return true;
    }
    else {
        return false;
...
threeMonthsLaterHoliday.setMonth(threeMonthsLaterHoliday.getMonth() + 4);
    ...
    try {
        ...
        if (pgName && pgName.toLowerCase().includes("floating holiday")) {
            ...
            if (startDate != null) {
                ...
                formContext.getAttribute("pg_leavedays").setValue(1);
            }
            return;
        }
    ...
    try {
        ...
        if (leavetype === 1) {
            if (endDate < startDate) {
                ...
                formContext.getControl("pg_enddate").setNotification("Start date must be less than or equal to end date", "validation");
...
function CalculateLengthOnchange(context) {
    ...
    if (startDate != null) {
        ...
        if (endDate != null) {
            ...
            //  var dateFormatEND = yearEn + "-" + monthEn + "-" + dayEn;
            var dateFormatEND = monthEn + "-" + dayEn + "-" + yearEn;
            if (leavetypevalue !== 6 && leavetypevalue !== 7) {
