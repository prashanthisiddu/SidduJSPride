...
////Requirement::when we click on that resolve button dialogbox need to diaplay,based on that values if it is yes condition bpf move to next stage///
...
////Requirement::Based on Country dojo field values should need to be display///
...
////Requirement::Get the loggedin user name and checking if he is a team member or not if he is team member then need to disable the filelds///
...
////Requirement::compare logged in user with owner of the record and if country is india then set the field values as disabled//
...
////Requirement::OnSave/Onload automatically move to next stage
...
function resolveifok(primaryControl) {   //mainform
  ...
  Xrm.Navigation.openConfirmDialog(confirmStrings, confirmOptions).then(
      ...
      function (error) {
          // Handle any errors that occur during the confirmation dialog
...
////Requirement::Based on condition show &hide field 
...
////Requirement::when we click on that resolve button dialogbox need to diaplay,based on that values if it is yes send a email and save&close///
